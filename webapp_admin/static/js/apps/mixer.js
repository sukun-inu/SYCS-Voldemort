...
/* 索引の波形は base64 の1バイト列（0〜255）で来る。素の JSON 配列だと同じ
   点数で7倍以上になり、2時間×5トラックで数MBの索引になってしまう。
   古いアーカイブは float の配列（stem.peaks）なので、そちらも読めるようにする。 */
function decodeSeries(encoded, legacy) {
  ...
  if (Array.isArray(legacy) && legacy.length) return Float32Array.from(legacy);
  return new Float32Array(0);
}

/* 1画素が受け持つ時間範囲の代表値を拾う。山は最大、実効値も最大を採る
   （平均にすると、静かな区間に挟まれた一言が消えて読めなくなる）。
   返すのは [山, 実効値] の2本で、長さはどちらも width。 */
function columnise(track, view, width) {
  const bucket = view.bucketSeconds || 0.05;
  const { peaks, rms } = track;
  const tops = new Float32Array(width);
  const cores = new Float32Array(width);
  if (!peaks.length) return { tops, cores };

  for (let x = 0; x < width; x += 1) {
    const from = view.start + x / view.pxPerSecond;
    const to = view.start + (x + 1) / view.pxPerSecond;
    let first = Math.floor(from / bucket);
    const last = Math.floor((to - 1e-9) / bucket);
    if (first < 0) first = 0;
    let top = 0;
    let core = 0;
    for (let b = first; b <= last && b < peaks.length; b += 1) {
      if (peaks[b] > top) top = peaks[b];
      if (b < rms.length && rms[b] > core) core = rms[b];
    }
    tops[x] = top;
    cores[x] = core > top ? top : core;
  }
  return { tops, cores };
}
...
export async function createMixer(container, options = {}) {
  ...
  stems.forEach((stem) => {
    stem.peakSeries = decodeSeries(stem.peaks_b64, stem.peaks);
    stem.rmsSeries = decodeSeries(stem.rms_b64, null);
  });
  const declared = Number(stems[0].bucket_seconds) || 0;
  const maxPoints = Math.max(0, ...stems.map((s) => s.peakSeries.length));
  ...
  stems.forEach((stem, index) => {
    ...
    const track = {
      ...
      name: stem.name, peaks: stem.peakSeries, rms: stem.rmsSeries,
