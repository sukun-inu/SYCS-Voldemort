...
ログの「共有キャッシュ」は3種類あります。止まっていた長さは、3つ目の行で分かります。

| 行 | 意味 |
| --- | --- |
| `共有キャッシュを 30 秒間使いません（…）` | 外れた。最初の1回だけ出る |
| `共有キャッシュにまだ繋がりません（落ちてから N 秒。…）` | 30秒ごとに試し直して、まだ繋がらない |
| `共有キャッシュが戻りました（N 秒ぶり）。` | 戻った。N は最初に外れてからの秒数 |

1つ目だけが出て3つ目が出ていないなら、まだ戻っていないか、戻ってから一度も
使われていません（リクエストが来ていない）。
