...
| 公開 API のレート制限 | 0 | `WEB_WORKERS` の数だけ独立して数えるので、上限が実質その倍通る |
| メトリクス | 0 | 3 プロセスぶんの死活と例外がどこにも集まらない |
| 管理画面のレート制限 | 1 | `ADMIN_WORKERS` の数だけ独立して数えるので、上限が実質その倍になる |

**無くても動きます。** `VALKEY_URL` を空にすると、今までどおりプロセス内の
キャッシュとプロセス内の計数で動きます（レート制限が消えるのではなく、ワーカー
ごとに数える形へ戻るだけです）。Valkey が動作中に落ちた場合も、遮断器が働いて
同じ状態へ落ちます（`services/shared_cache.py` の冒頭に
...
| `FORECAST_INTERVAL_WIDTH_MULTIPLIER` | 任意 | 予測区間の幅にかける補正係数の**上限**（既定 `1.35`、サンプルが少ない局面で適用）。重複ありの7日リターンは強く自己相関するため標本分位点が裾を過小評価する。本番実データ102点で較正し、補正なし66.7%→補正後81.4%の被覆率になった |
| `FORECAST_INTERVAL_WIDTH_MULTIPLIER_MIN` | 任意 | 同係数の**下限**（既定 `1.10`）。7日リターンが43本→150本と増えるにつれ上限からこの値まで逓減する。固定のままだとデータ蓄積に伴い被覆率が名目80%に対し91%まで広がってしまうため |
