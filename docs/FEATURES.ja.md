...
- ZIP には各人の mp3 に加えて `info.txt` / `info.json`（チャンネル・開始者・長さ・発話時間・
  受信の内訳）と、ミキサー用の索引 `mixer.json`（トラックの並び・波形・長さ）が入る
- **切り分け用に、届いたパケットそのものも `packets.jsonl` に控える**（1行1枚、
  詰め物を剥がす前・復号する前のまま）。枚数は絞ってあり、冒頭と「普通の音声として
  扱えなかったもの」だけが入る。info.txt には 0 でないカウンタだけが出るので、
  その行が出ていること自体が「見るべきものがある」の合図になる
...
- **ミキサー（管理画面）**: 録音を DAW のように扱える。画面は2つ
  ...
  - **区間の指定** … 決め方は3通り。時間目盛りの横ドラッグ、**帯の端をつまんで伸縮**、
    **`[` `]` で再生位置を端にする**。決まった区間は操作列に
    「区間 0:05.00 〜 0:12.00（7.00 秒）」と出る。解除は時間目盛りの上を1回
    クリックするか `Esc`。全体表示では 1px が数秒になる（6時間なら1px＝約7秒）ので、
    秒の単位で合わせたいときは ＋ で拡大してからドラッグする。丸めと下限（0.05秒）
    の判断は1箇所に置いてあり、どの決め方でも同じ結果になる
  - **区間の書き出し** … 決めた区間だけを全トラック分 ZIP で落とせる。
    無圧縮で入っている mp3 をフレーム境界で切るだけ（再エンコードしない）ので速く、
    音も劣化しない。切り出しても時間軸は揃ったまま
  - キーボード: スペースで再生／停止、←→ で移動（Shift で10秒）、Home で先頭、
    ＋− で拡大、`[` `]` で区間の端、`Esc` で区間の解除
  - 波形は**表示している範囲だけ**描く。6時間を 40px/秒 で並べると 86万px になり、
    キャンバスの幅の上限（32767px 前後）を超えるため
  - 波形の目盛りは 0.05 秒刻みで拾い、点数が増えすぎないよう間引く（上限
    200,000 点。2時間45分までは間引かない）。**間引き幅は
  ...
  - 波形は**山と実効値（RMS）の2層**で描く。薄い層が山、濃い層が実効値。山だけ
    だと、人の声は瞬間的な山が揃いやすいのでどこも同じ高さに見え、密度の差が
    読めない。1画素ずつ縦線で塗る（多角形1本で塗ると隣の画素と補間されて鈍る）。
    無音の区間には何も描かない（中心線を通すと、無音とごく小さい音が見分け
    られなくなる）
...
- **ジッタバッファの差し替え**: 拡張のジッタバッファは、順番待ちの穴が空いた瞬間に
  **抱えていたパケットのうち先頭1個だけを返し、残りを捨てる**（穴を待つ側と
  `timeout=0` で取りに来る側が噛み合っていない）。捨てられたぶんは受信側まで
  来ないので、並べ直しでは取り返せない。本番のログでは録音中ずっと
  `1 packets were lost being flushed in decoder-1148` が出続け、話者が
  入れ替わった直後には1回で6個捨てていた。**捨てない版へ差し替える**
  （`services/voice_jitter.py`）。取り出す順序は変えない
- **受信パケットの組み立て**: 上のとおり順不同で出てくるので、そのまま Opus に
  渡すと音が壊れる。受信側でもう一度並べ直してから復号している
