...
- **ミキサー（管理画面）**: 録音を DAW のように扱える。画面は2つ
  ...
  - **区間の指定** … 決め方は4通り。時間目盛りの横ドラッグ、**帯の端をつまんで伸縮**、
    **開始と終了の欄へ直接入力**（`1:23.45` / `83.45` のどちらでも）、
    **`[` `]` で再生位置を端にする**（ボタンは「ここから」「ここまで」）。
    全体表示では 1px が数秒になる（6時間なら1px＝約7秒）ので、ドラッグだけでは
    秒の単位に届かない。丸めと下限（0.05秒）の判断は1箇所に置いてあり、
    どの決め方でも同じ結果になる
