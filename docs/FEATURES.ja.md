...
- **E2EE（DAVE）の通話は録音できない**: Discord は 2024年9月から音声を端から端まで
  暗号化しており、2026年3月2日からボットにも必須になった。受信側でこれを復号する
  実装は discord.py にも discord-ext-voice-recv にも無い（discord.py の DAVE は送信専用）
  - 暗号文を Opus に食わせると、多くは例外にならず**雑音が返る**。「録れているのに
    聞けない」という壊れ方になるため、フレーム末尾のマーカー（`0xFAFA`）で検出して
    入り口で外し、割合が高ければ理由を添えて停止する
