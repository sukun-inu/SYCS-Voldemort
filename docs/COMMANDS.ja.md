...
| `/tts leave` | Bot を VC から退出させキューをクリア（`/tts join` 中なら元の設定に戻る）。**録音中なら締めてリンクを出してから退出** |
...
- 開始 … 対象 VC への入室 / `/record start` / `/tts join` / 読み上げの自動参加
- 停止 … `/record stop` / `/tts leave` / VC が無人になる / 上限時間に到達 / Bot の停止
