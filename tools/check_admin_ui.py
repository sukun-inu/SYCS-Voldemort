...
_SAMPLE_USER = {
    ...
    "roles": [{"id": 1, "name": "常連"}],
    "abilities": {"trusted_user": True},
    "last_event_type": "member_join",
    "last_event_at": datetime(2026, 8, 21, 3, 0, tzinfo=timezone.utc),
    "first_seen_at": None,
    "last_joined_at": None,
    "last_left_at": None,
    "updated_at": None,
...
_SAMPLE_EVENT = {
    ...
    "actor_name": None,
    "reason": None,
    "event_at": datetime(2026, 8, 21, 3, 0, tzinfo=timezone.utc),
    "payload": {},
}
...
def main():
    ...
    with sync_playwright() as p:
        ...
        context.add_cookies([{"name": "admin_session", "value": session_cookie(), "domain": "127.0.0.1", "path": "/"}])
        ...
        check(
            f"タイルが{expected_tiles}枚（開発者パネル・SQL エディタ込み）",
            tiles.count() == expected_tiles,
            str(tiles.count()),
        )
        ...
        check(
            "タイトルが入る",
            page.locator(".window-title").inner_text() == "ログ設定",
        ...
        check("保存ボタンは初期状態で無効", page.locator(".savebar .btn-primary").is_disabled())

        # ── 明示保存 ──
        page.select_option('.window .field[data-key="log_level"] select', "DEBUG")
        page.wait_for_timeout(100)
        check("変更すると保存ボタンが有効になる", not page.locator(".savebar .btn-primary").is_disabled())
        check("未保存の印がタイトルバーに出る", page.locator(".window-dirty").is_visible())
        check(
            "状態表示が変わる",
            "未保存の変更 1 件" in page.locator(".savebar-status").inner_text(),
        ...
        check(
            "保存でトーストが出る",
            "保存しました" in page.locator(".toast").inner_text(),
            page.locator(".toast").inner_text(),
        )
        check(
            "保存後は変更なしに戻る",
            page.locator(".savebar-status").inner_text() == "変更はありません",
            page.locator(".savebar-status").inner_text(),
        )
        check("サーバー側にも反映されている", get_log_settings(999).get("level") == "DEBUG", str(get_log_settings(999)))

        # ── 一覧が取得できない環境では手入力に切り替わる ──
        check(
            "チャンネル欄が手入力にフォールバックする",
        ...
        panel_ids = page.eval_on_selector_all(".app-tile", "els => els.map(e => e.dataset.appId)")
        page.keyboard.press("Escape")
        already = set(page.eval_on_selector_all(".window", "els => els.map(e => e.dataset.appId)"))
        ...
        for app_id in panel_ids:
            ...
            check(f"{app_id} を開いただけでは変更なし", bar.inner_text() == "変更はありません", bar.inner_text())
            check(f"{app_id} に未保存の印が出ていない", not page.locator(window + ".window-dirty").is_visible())
            page.click(window + ".window-control.close")
        ...
        for app_id in ("djaudio", "welcome", "tts"):
            ...
            if overlap.get("missing"):
                check(
                    f"{app_id} の保存バーが中身に重ならない",
                    False,
                    ".panel-scroll が無い（保存バーがまた中身と一緒に流れている）",
                )
                page.click(f'.window[data-app-id="{app_id}"] .window-control.close')
                continue
            check(f"{app_id} の保存バーが中身に重ならない", overlap["hidden"] == [], f"隠れている: {overlap['hidden']}")
            check(
                f"{app_id} は中身だけが送られる（窓ごと送られない）",
                overlap.get("bodyOverflow") == 0,
                f"はみ出し {overlap.get('bodyOverflow')}px",
            )
            check(f"{app_id} の保存バーが中身の下に居る", overlap.get("barBelowContent") is True)
        ...
        check(
            "追加内容が表示される",
            "生成AI" in page.locator(feed_window + ".list-row").inner_text(),
        ...
        page.wait_for_function(
            """() => document.querySelector('.window[data-app-id="news-feeds"] .list-row')
                     ?.textContent.includes('更新後')""",
            timeout=5000,
        )
        check(
            "更新すると更新したと伝える",
            "更新しました" in page.locator(".toast").last.inner_text(),
        ...
        check(
            "矢印キーでウィンドウが小さくなる",
            resized["shrunk"]["w"] < resized["before"]["w"] and resized["shrunk"]["h"] < resized["before"]["h"],
            f"{resized['before']} -> {resized['shrunk']}",
        )
        check(
            "反対の矢印キーで大きくなる",
            resized["grown"]["w"] > resized["shrunk"]["w"] and resized["grown"]["h"] > resized["shrunk"]["h"],
        ...
        check(
            ...
            (
                ...
                f'{int(moved["x"] + moved["width"])},{int(moved["y"] + moved["height"])}'
                if grip
                else "つまみ無し"
            ),
        )
        ...
        check("最小化するとタスクバーが薄くなる", page.locator(".taskbar-app.is-minimized").count() == 1)
        ...
        page.wait_for_timeout(200)
        check("未保存なら閉じるのを止められる", page.locator('.window[data-app-id="logging"]').count() == 1)
        ...
        check("監視履歴も出る", page.locator('.window[data-app-id="monitoring"] .section').count() == 2)
        ...
        check(
            "ユーザー一覧が描画される",
            page.locator(us + ".list-row").count() == 2,
            f'{page.locator(us + ".list-row").count()} 行',
        )
        check("iframe ではない", page.locator(us + "iframe").count() == 0)
        check(
            "状態がラベル化されている",
            "在籍" in page.locator(us + ".list-row .chip").first.inner_text(),
        ...
        check(
            "サーバー側で絞り込める",
            page.locator(us + ".list-row").count() == 1,
            f'{page.locator(us + ".list-row").count()} 行',
        )
        ...
        page.wait_for_function("""() => document.querySelectorAll('.log-pane .log-line').length > 0""", timeout=8000)
        check(
            "ログはタブを開くだけで読み込まれる",
        ...
        check("重大度で色が分かれる", all(n > 0 for n in shape["levels"]), f'エラー/警告/デバッグ = {shape["levels"]}')
        ...
        check(
            "既定は読み取り専用",
            "読み取り専用" in page.locator(sql + ".sql-mode").inner_text(),
        ...
        check(
            "アイコンがスプライトから描画される",
            icon_count > 10 and box and box["width"] > 0,
        ...
        check(
            "設定ページへの直接アクセスがデスクトップに集約される",
            page.locator('.window[data-app-id="djaudio"]').count() == 1 and "open=" not in page.url,
            page.url,
        )
        ...
        check(
            "通知の中身が1行目の中心に並ぶ",
            abs(toast["icon"]) <= 1 and abs(toast["close"]) <= 1 and abs(toast["above"] - toast["below"]) <= 1,
        ...
        check(
            "ログイン画面",
            anon_page.locator(".login-card").is_visible() and anon_page.locator(".icon").count() > 0,
        ...
        motion.add_cookies([{"name": "admin_session", "value": session_cookie(), "domain": "127.0.0.1", "path": "/"}])
        ...
        check(
            ...
            and moving["taskbar"] == "taskbar-rise"
        ...
        calm.add_cookies([{"name": "admin_session", "value": session_cookie(), "domain": "127.0.0.1", "path": "/"}])
        cpage = calm.new_page()
        ...
        narrow.add_cookies([{"name": "admin_session", "value": session_cookie(), "domain": "127.0.0.1", "path": "/"}])
