...
# 声の高さと声道長の食い違いを、どちら向きに・どれだけ離れたら疑うか。
#
# 以前は「絶対値で 4cm 以上ずれていたら加工」だった。これが誤検知の主因
# だった。**同じ人でも声の高さは声域で倍近く動くが、声道の長さは動かない。**
# 興奮・笑い・叫びで F0 が上がると、この式は「体が小さいはず」と読み、
# 実際の声道長との差を食い違いとして数える。
#
# 向きで分かれることに気づけば分離できる。
#
#   声域を上げた地声   F0 が上がる／声道長はそのまま → 期待値より「長い」側へずれる
#   高いほうへの加工   F0 も声道長も同じ倍率で動く   → 期待値より「短い」側へずれる
#
# 実測（tests/test_services.py::VoiceChangerJudgementTests が同じ組を持つ）。
# 差 = 実測 − 期待:
#
#   地声  成人男性 叫び    F0 280 / 17.5cm   +4.56   ← 長い側
#   地声  成人女性 叫び    F0 350 / 15.2cm   +4.15   ← 長い側
#   地声  子ども 低い声で  F0 160 / 12.0cm   -4.18
#   地声  子ども かなり低く F0 130 / 12.0cm   -4.99   ← 短い側の最悪
#   加工  実録音の検出例   F0 118 / 10.8cm   -6.52   ← 捕まえたい下限
#   加工  実録音の検出例   F0 118 /  9.5cm   -7.82
#
# 長い側を見ないことで、地声の誤検知（上2件）が消える。短い側の閾値を
# 5.5cm に置くと、地声の最悪 -4.99 と、捕まえたい -6.52 の間に入る
# （測り方の精度は ±0.5cm 程度なので、両側に余裕がある）。
#
# 上の8本の地声で誤検知 0、絶対値 4cm のころは 3本が加工扱いだった。
# 代わりに、男性を 1.75 倍した音（-4.83cm）は捕まらなくなる。ただしこれは
# 声道長が 10.0cm と人の範囲の下端そのもので、1.8 倍以上なら 10cm を割って
# 範囲の規則で捕まる。狭い帯だけの取りこぼしと引き換えに、地声を疑わない。
_VTL_F0_SHORTFALL_CM = 5.5
...
def _judge(vtl: float, f0: float, expected_vtl: float) -> tuple[str, list[str]]:
    ...
    elif expected_vtl and vtl and (expected_vtl - vtl) > _VTL_F0_SHORTFALL_CM:
        # 声の高さのわりに声道が「短い」ときだけ見る（長い側は見ない。理由は
        # 定数のところに書いた）。
        reasons.append(
            f"声の高さ（{f0:.0f} Hz）のわりに声道長 {vtl:.1f} cm は短すぎます"
