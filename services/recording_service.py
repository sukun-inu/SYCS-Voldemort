...
def _receive_breakdown(session: RecordingSession) -> dict:
    ...
    for row in by_speaker.values():
        ...
        # 届かなかったぶんはそこに入っていない。
        # **届いたのに書けなかったぶんも入れること。** 手遅れ（並べ直しの位置を
        # もう出し終えたあとに届いた）と溢れ（持ち主が分からないまま抱えきれずに
        # 捨てた／停止まで分からなかった）は、音は届いているのにトラックへ1バイトも
        # 書かれない。詰め物を外したのは「はじめから音が入っていない」からで、
        # こちらは理由が逆になる。
        #
        # 分母はそのままでよい。received は push() へ渡す前に数えるので、手遅れも
        # 溢れも既に入っている。
